...
public class ModMoss extends ModBoolean {
    ...
    // ModifyBuilder.registerModifier(new ModInteger(new ItemStack[] { new ItemStack(TinkerTools.materials, 1, 6) }, 4,
    // "Moss", 3, "\u00a72", StatCollector.translateToLocal("modifier.tool.mossnew")));
    ...
    public static float mossChance(Entity entity) {
        ...
        if (!world.provider.hasNoSky && world.isDaytime()) {
            ...
            if (angle < (float) Math.PI) angle += (0.0F - angle) * 0.2F;
            else angle += ((float) Math.PI * 2F - angle) * 0.2F;
