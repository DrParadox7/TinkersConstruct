            ...
            if (logic != null && logic.xCoord == this.x && logic.yCoord == this.y && logic.zCoord == this.z)
                ...
                else
                {
                    ...
                    if (stackBlank != null && stackBlank.stackSize > 0 && MoldBuilder.isBlank(stackBlank))
                    {
                        ...
                        if (warning)
                            TConstruct.logger.warn("Possible packet-cheating with MoldingTable for player " + player.getCommandSenderName());
                        else
                            logic.setSelectedMold(this.contents);
                    }
                }
        }
    }
