...
public class PHConstruct {
    ...
    public static void initProps(File location) {
        ...
        modModifier = config.get(
                "Balance",
                "Xp cost modifier per active modifier. [Formula: XP cost = (repair * (1 + Modifiers * modModifier))/repairPerXp]",
                1.0,
                "Set to 0 to disable").getDouble(1.0);
        modifierCost = config.get("Balance", "Flat Xp cost for applying modifier", 10, "Set to 0 to disable")
                .getInt(10);
        repairModifier = config.get("Balance", "Increase in XP cost per repair count", 0, "Set to 0 to disable")
                .getInt(0);
        balancedPartCrafting = config
                .get("Balance", "Limits Stencils to only early game tool parts", true, "Set to false to disable")
                .getBoolean(true);
        moltenStone = config
                .get("Balance", "Allow Stone to be molted into smeltery blocks", true, "Set to false to disable")
                .getBoolean(true);
        ...
        experimentalBackports = config.get("Experimental", "Enable experimental backports", true).getBoolean(true);
