...
public class AbilityHelper {
    ...
    public static boolean onBlockChanged(ItemStack stack, World world, Block block, int x, int y, int z,
        ...
        if (tags.getCompoundTag("InfiTool").hasKey("Moss")) mossChance = ModMoss.mossChance(player);
    ...
    public static void damageTool(ItemStack stack, int dam, NBTTagCompound tags, EntityLivingBase entity,
        ...
        if (tags.hasKey("InfiTool") && dam > 0) // unbreaking only affects damage, not healing
        {
            ...
            if (toolTags.hasKey("Moss") && entity != null) {
                ...
                for (int i = 0; i < dam; i++) if (random.nextFloat() < mossChance) dam--;
    ...
    private static boolean equalityOverrideLoaded = Loader.isModLoaded("CoFHCore"); // Mods should be loaded far enough
                                                                                    // before this is ever initialized
