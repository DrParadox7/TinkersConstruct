...
public class XpUtils {
    ...
    public static void deductXP(int amount, EntityPlayer player) {
        ...
        int effectiveTotal = getXPFromLevel(player.experienceLevel)
                + (int) (player.experience * (float) xpBarCap(player.experienceLevel));
    ...
    public static int XPRepair(int repairValue, int upgrades, int repairCount) {
        ...
        if (repairPerXp > 0) {
            int XpCost = (int) ((repairValue * (1 + upgrades * modModifier)) / repairPerXp)
                    * (1 + repairCount * repairModifier);
            return XpCost;
        } else return 0;
    }
    ...
    public static int ModCost() {
        ...
        if (cost > 0) return getXPFromLevel(cost);
        else return 0;
    }
}
